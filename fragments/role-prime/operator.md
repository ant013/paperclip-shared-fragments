...
- palace.code.get_architecture(project="{{CODEBASE_MEMORY_PROJECT}}") — broad project structure
- palace.code.search_graph(name_pattern="...", project="{{CODEBASE_MEMORY_PROJECT}}") — find function/class by name
- palace.code.trace_call_path(function_name="...", project="{{CODEBASE_MEMORY_PROJECT}}", mode="callers"|"callees") — call chains
- palace.code.get_code_snippet(qualified_name="<{{CODEBASE_MEMORY_PROJECT}}.path>", project="{{CODEBASE_MEMORY_PROJECT}}") — read source
...
- palace.code.query_graph(query="MATCH ... RETURN ...", project="{{CODEBASE_MEMORY_PROJECT}}") — Cypher
